...
export default class HomeScreen extends React.Component {
  ...
  GetData(page) {
    return fetch(
      config.baseurl + `produk?type=json&page=${page}&search=${this.state.search}`,
    )
  ...
  GetProdukKategori() {
    return fetch(config.baseurl + 'api/produkperkategori')
      .then((response) => response.json())
      .then((responseJson) => {
        ...
        this.setState(
          {
            ...
            refreshing: false,
            isLoading: false,
            dataSourceKategori: responseJson,
          },
          function () {},
        );
      })
        ...
        <ScrollView
          ...
          <View style={styles.Card}>
            {this.state.isLoading ? (
              ...
              <View>
                {this.state.dataSourceKategori.map((data) => (
                  <View>
                  <View style={{flexDirection:'row'}}>
                    <Text style={styles.Deskripsi}>{data.nama_kategori} </Text>
                    <Text 
                      ...
                      renderItem={({item}) => (
                        <TouchableOpacity
                          ...
                          <Text
                            ...
                            {item.nama_produk.length > 15 ? item.nama_produk.substring(0, 15) + '...' : item.nama_produk}
                          </Text>
